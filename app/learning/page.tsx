...
export default function LearningPage() {
  ...
  const handleGenerateRecommendations = async () => {
    ...
    try {
      ...
      if (user && user.id) {
        try {
          await saveLearningRecommendation(user.id, {
            userId: user.id,
            recommendedCourses: result.recommendedCourses || recommendedCourses,
            skillGaps: result.skillGaps || skillGaps,
          ...
          // Don't halt the process if saving fails
        }
      }
      ...
      setError(err.message || "Failed to generate learning recommendations. Please try again or use a different file.");
      
      // We still set the generated flag to true so we show some recommendations
      setHasGeneratedRecommendations(true);
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-muted-foreground">
                Personalized learning recommendations to build in-demand skills
              </p>
            </div>
            <div className="flex flex-col md:flex-row gap-2">
              <Button variant="outline" asChild>
                <Link href="/learning/recommendations">
                  <BookOpen className="mr-2 h-4 w-4" /> My Learning Plans
                </Link>
              </Button>
